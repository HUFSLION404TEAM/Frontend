...
export default function Portfolio({ anchorRef, stickyGap = 76, items = [] }) {
  return (
    <>
      <div ref={anchorRef} style={{ height: 1, scrollMarginTop: stickyGap }} />
      ...
      {items.map((p) => (
        <PItem key={p.id}>
          <Row style={{ justifyContent: "space-between" }}>
            <Meta>{p.brand}</Meta>
            <Badge><Thermo /> {p.temp}°C</Badge>
          </Row>

          <PTitle>{p.title}</PTitle>

          <Thumbs>
            <Thumb>BLACK<br/>FRIDAY</Thumb>
            <Thumb>BLACK<br/>FRIDAY</Thumb>
          </Thumbs>
        </PItem>
      ))}
