...
import {
  C, Page, TopBar, BackBtn, Title, ProgressWrap, Progress, Body, Bottom,
  NextBtn, Actions, OutlineBtn, PrimaryBtn, TopTitle
...
const TYPES = ["제조업","도매업","소매업","서비스업","건설업","IT업","금융업","교육업","의료업"];
...
export default function OnboardingOwner() {
  ...
  const valid = useMemo(() => {
    switch (step) {
      ...
      case 2: return form.location.trim().length > 3;
      case 3: return phoneRegex.test(form.phone.trim());
      case 4: return bizRegex.test(form.bizNo.trim());
      case 5: return form.intro.trim().length > 5;
      case 6: return true;
    ...
    <Page>
      <TopBar>
        <BackBtn onClick={back} disabled={step === 0}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
            <path fillRule="evenodd" clipRule="evenodd" d="M18.3239 5.00858C18.5427 5.22736 18.6655 5.52405 18.6655 5.83341C18.6655 6.14277 18.5427 6.43946 18.3239 6.65824L10.9821 14.0001L18.3239 21.3419C18.5365 21.5619 18.654 21.8566 18.6514 22.1625C18.6487 22.4684 18.526 22.7611 18.3097 22.9774C18.0934 23.1937 17.8008 23.3164 17.4949 23.319C17.189 23.3217 16.8943 23.2041 16.6743 22.9916L8.5076 14.8249C8.28888 14.6061 8.16602 14.3094 8.16602 14.0001C8.16602 13.6907 8.28888 13.394 8.5076 13.1752L16.6743 5.00858C16.893 4.78986 17.1897 4.66699 17.4991 4.66699C17.8085 4.66699 18.1052 4.78986 18.3239 5.00858Z" fill="#1A96FE"/>
          </svg>
        </BackBtn>
      ...
      */}
      ...
      <Body>
        ...
        {step === 1 && <StepName value={form.name} onChange={onChange("name")} />}

        {step === 2 && <StepLocation value={form.location} onChange={onChange("location")} />}

        {step === 3 && <StepPhone value={form.phone} onChange={onChange("phone")} />}

        {step === 4 && <StepBizNo value={form.bizNo} onChange={onChange("bizNo")} />}

        {step === 5 && <StepIntro value={form.intro} onChange={onChange("intro")} />}
      ...
      <Bottom>
        {step < total - 1 ? (
          ...
          <Actions>
            <OutlineBtn disabled={loading} onClick={() => setStep(0)}>수정</OutlineBtn>
